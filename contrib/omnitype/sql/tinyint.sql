...
-- 显示使用 BTree 索引
EXPLAIN SELECT * FROM tinyint_test WHERE value >= 100; -- Index Cond: (value >= 100)
EXPLAIN SELECT * FROM tinyint_test WHERE 100 >= value;  -- Index Cond: (value <= 100)，必须存在 integer <= tinyint 操作符，否则会报错

-- 显示使用 Hash 索引
EXPLAIN SELECT * FROM tinyint_test WHERE value = 120;
